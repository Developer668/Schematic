...
type Analysis = { possibleExternalTools: string[]; claims: { filesRead: false; filesExecuted: false; filesImported: false; compatibilityVerified: false }; steps: Step[] };
...
export default function ImportDialog({ onClose }: { onClose: () => void }) {
  ...
  const doImportProject = () => {
    ...
    try {
      ...
      // Capacity failures are deliberately raised before the Zustand state is
      // committed. Keep the dialog open so the user can export/recover the
      // room or delete an older project instead of losing the selected file.
      setError(cause instanceof Error ? cause.message : "The project could not be imported. No project was changed.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="import-title">
      <div ref={dialogRef} aria-busy={loading} className="w-full max-w-2xl overflow-hidden rounded-xl border border-border bg-card shadow-2xl"><div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-4"><div><h2 id="import-title" className="text-base font-semibold">Import into Schematic</h2><p className="mt-1 text-xs text-muted-foreground">Open a project or inspect hardware reference-file compatibility.</p></div><button ref={closeRef} type="button" className="workspace-icon-button" aria-label="Close import" onClick={onClose}><X size={14} /></button></div>
        ...
        {mode === "project" ? (
          ...
          <div role="tabpanel" aria-busy={analysisLoading} className="space-y-4">
            <p className="text-xs leading-relaxed text-muted-foreground">Inspect whether SPICE, IBIS, Touchstone, Verilog, source, CMSIS, FMU, STEP, or glTF/GLB files have recognized import metadata. Files are not executed and this analysis does not change the component catalog.</p>
            ...
            {error && <div className="flex items-start gap-2 rounded-md border border-red-500/25 bg-red-500/10 p-2 text-xs text-red-600 dark:text-red-300" role="alert"><AlertCircle size={14} className="mt-0.5 shrink-0"/><span>{error}</span></div>}
            {analysis && <div role="status" className="rounded-lg border border-border bg-muted/15 p-3 text-xs"><div className="font-medium">Filename-only external handoff hints</div><div className="mt-1 text-muted-foreground">{analysis.possibleExternalTools.join(", ") || "No recognized external tool hint"} · suggestions only, not installed or verified in Schematic</div>{analysis.steps.length > 0 && <ol className="mt-3 space-y-1 border-t border-border pt-3">{analysis.steps.map((item) => <li key={item.step} className="flex gap-2"><span className="font-mono text-muted-foreground">{item.step}.</span><span>{item.label}</span>{item.detail && <span className="text-muted-foreground">— {item.detail}</span>}</li>)}</ol>}</div>}
