...
function boardFqbnFor(definitionId: string) {
  return definitionId.includes("esp32") ? "esp32:esp32:esp32" : definitionId.includes("pico") ? "rp2040:rp2040:rpipico" : "arduino:avr:uno";
}
...
export default function MonacoWorkspace() {
  ...
  const isBoard = Boolean(active && (active.definitionId.includes("arduino") || active.definitionId.includes("esp32") || active.definitionId.includes("pi") || active.definitionId.includes("pico")));
  ...
  useEffect(() => {
    ...
    const target = project.firmwareTargets.find((item) => item.componentId === active.id);
    const nextCode = target?.files[0]?.content ?? DEFAULT_SKETCH;
    syncingCodeRef.current = true;
    setCode(nextCode);
    setCodeIssues(validateFirmwareFiles(target?.files ?? [{ name: "sketch.ino", content: nextCode }]));
    queueMicrotask(() => { syncingCodeRef.current = false; });
  }, [active, isBoard, project.firmwareTargets, setCodeIssues]);

  const handleCodeChange = (value: string) => {
    setCode(value);
    if (!active || !isBoard || syncingCodeRef.current) return;
    const currentTarget = useProjectStore.getState().project.firmwareTargets.find((target) => target.componentId === active.id);
    const files = [{ name: currentTarget?.files[0]?.name ?? "sketch.ino", content: value }];
    updateFirmware(active.id, files, { boardFqbn: currentTarget?.boardFqbn ?? boardFqbnFor(active.definitionId) });
    setCodeIssues(validateFirmwareFiles(files));
  };

  const doCompile = async () => {
    if (!active || !isBoard) return;
    const boardFqbn = boardFqbnFor(active.definitionId);
    setCompiling(true);
    setCompile({ status: "checking", boardFqbn, log: "Checking source…", checkedAt: Date.now() });
    const currentTarget = useProjectStore.getState().project.firmwareTargets.find((target) => target.componentId === active.id);
    updateFirmware(active.id, [{ name: currentTarget?.files[0]?.name ?? "sketch.ino", content: code }], { boardFqbn: currentTarget?.boardFqbn ?? boardFqbn });
    try {
      await invokeWebMCPTool("firmware.compile", { componentId: active.id, boardFqbn: currentTarget?.boardFqbn ?? boardFqbn });
    } catch (error) {
  ...
  if (!active || !isBoard) {
    return (
      <div className="h-full flex flex-col bg-card">
        ...
        <div className="flex-1 min-h-[160px] relative"><Editor height="100%" theme={isDark ? "vs-dark" : "light"} defaultLanguage="cpp" value={code} options={EDITOR_OPTIONS} onChange={(value) => setCode(value ?? "")} /></div>
    ...
    <div className="h-full flex flex-col bg-card">
      <div className="h-8 flex items-center justify-between px-2 border-b border-border bg-muted/20 shrink-0 gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => setCompile({ status: "idle" })} className="hidden sm:inline-flex text-xs px-2 py-1 rounded border border-border hover:bg-muted">Clear</button>
          <button className="text-xs px-2.5 py-1 rounded bg-primary text-primary-foreground hover:bg-primary/90 font-medium flex items-center gap-1" onClick={doCompile} disabled={compiling}><Play size={10} className="fill-white" /> {compiling ? "Compiling…" : "Compile"}</button>
        </div>
      </div>
      <div className="flex-1 min-h-[120px] relative"><Editor height="100%" theme={isDark ? "vs-dark" : "light"} defaultLanguage="cpp" value={code} onChange={(value) => handleCodeChange(value ?? "")} options={EDITOR_OPTIONS} /></div>
      {compile.log && compile.status !== "idle" && (
        <div className="border-t border-border max-h-[140px] flex flex-col shrink-0">
          <div className="h-6 flex items-center justify-between px-2 bg-[#0a0a0a] border-b border-zinc-800"><span className="text-[11px] font-mono text-zinc-400">Output · {compile.status}</span><button onClick={() => setCompile({ status: "idle" })} className="text-zinc-400 hover:text-white p-1"><Trash2 size={10} /></button></div>
